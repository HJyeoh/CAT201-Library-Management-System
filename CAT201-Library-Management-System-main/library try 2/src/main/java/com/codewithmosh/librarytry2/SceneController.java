...
public class SceneController {
    ...
    public void displayLibraryBook() {
        ArrayList<Book> books = Book.displayLibraryBook();
    ...
    public void deleteBook() {
        String title = addTitleField.getText();
        String isbn = addIsbnField.getText();
        Boolean enterAuthor = !addAuthorField.getText().isEmpty();

        isDelete = Library.deleteBook(isbn,title);

        if (isDelete) {
            showMessage(deleteSuccessLabel, "Book deleted successfully!",true);
        }else if(enterAuthor) {
            showMessage(deleteFailLabel, "Cannot delete book by Author",false);
        }else{
