...
public class Library {
    ...
    static ArrayList<Book> books = Main.getLibraryBookData();
    ...
    public static boolean deleteBook(String isbn, String title) {
        for (Book book : books) {
            if (book.getIsbn().equalsIgnoreCase(isbn) || book.getTitle().equalsIgnoreCase(title)) {
                books.remove(book);
                Main.saveBooksToFile();
